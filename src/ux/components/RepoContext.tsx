...
export function RepoContextBox({ ctx, provider }: Props): JSX.Element {
  ...
  const isProtected =
    ctx.branch === 'main' || ctx.branch === 'master' || ctx.branch === 'develop';

  const aheadStr = ctx.aheadCount > 0
    ? <Text color={theme.success}> ↑{ctx.aheadCount}</Text>
    : null;
  const behindStr = ctx.behindCount > 0
    ? <Text color={theme.error}> ↓{ctx.behindCount}</Text>
    : null;
