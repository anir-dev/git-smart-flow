...
export function ValidationReport({ repoName, sections }: Props): JSX.Element {
  ...
  return (
    <Box flexDirection="column" width={width}>
      <Text bold color="white">Validación del repositorio — {repoName}</Text>
      ...
      <Box marginTop={1}>
        {allOk
          ? <Text bold color={theme.success}>✔  Sin problemas detectados</Text>
          : <Text bold color={theme.warning}>⚠  Revisa los elementos marcados antes de continuar</Text>
        }
      </Box>
    </Box>
  );
