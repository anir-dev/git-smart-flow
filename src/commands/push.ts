...
async function runInkPush(): Promise<void> {
  ...
  const recentCommits = upstream
    ? getCommitsSinceBase(upstream, cwd).slice(0, 5)
    : [];
  ...
  const options = [
    { label: isProtected ? '⚠️  Sí, hacer push (rama protegida)' : 'Sí, hacer push', value: 'push' },
  ...
  function PushUI({ onChoice }: { onChoice: (val: string) => void }): JSX.Element {
    ...
    return React.createElement(Box, { flexDirection: 'column', paddingX: 1, width },
      ...
      React.createElement(Box, {
        ...
        paddingX: 1, marginBottom: 1, width: width - 2,
      },
        React.createElement(Text, null,
        ...
        React.createElement(Text, null,
          ...
          React.createElement(Text, { color: ahead > 0 ? theme.info : theme.muted }, `${ahead} commits pendientes`),
        ),
        behind > 0
          ? React.createElement(Text, { color: theme.warning }, `  ⚠ Behind por ${behind} commits — considera pull primero`)
          : null,
      ),
  ...
  const choice = await renderInteractive<string>((resolve) =>
    React.createElement(PushUI, { onChoice: resolve }) as JSX.Element
  );

  if (choice !== 'push') { info('Push cancelado.'); return; }
...
async function runPlainPush(): Promise<void> {
  const cwd = process.cwd();
  const config = getConfig();
  const branch = getCurrentBranch(cwd);
  ...
  if (ahead === 0 && upstream) { info('Nothing to push — already up to date.'); return; }
