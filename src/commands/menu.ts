...
import { blank, error, header, info, keyValue, section, success, warning } from '../ux/display.js';
...
async function printStatus(cwd: string): Promise<void> {
  ...
  if (lastCommit) {
    const msg =
      lastCommit.message.length > 50 ? lastCommit.message.slice(0, 48) + '…' : lastCommit.message;
...
async function handleChoice(choice: string): Promise<void> {
  switch (choice) {
    ...
    case '3': {
      const { runCommitMessage } = await import('./commit-message.js');
      await runCommitMessage({});
      break;
    }
    case '4': {
    ...
    case 'u': {
      const { runRevert } = await import('./revert.js');
      await runRevert();
      break;
    }
    case 'l': {
    ...
    case '9': {
      const { runAliases } = await import('./aliases.js');
      await runAliases();
      break;
    }
    case 'r': {
      const { runRepoInit } = await import('./repo-init.js');
      await runRepoInit();
      break;
    }
    case 'd': {
